...
import { Sun as IconSun, Moon as IconMoon, Layers as IconLayers, EyeOff as IconEyeOff } from 'lucide-react'
...
const Floorplan3DView: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative overflow-hidden bg-slate-950">
      ...
      {/* Loading overlay */}
      {!sceneReady && (
        <div className="absolute inset-0 flex items-center justify-center bg-slate-900 text-white/70 text-sm font-medium z-10">
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
        <div className="bg-base-100/60 backdrop-blur-md p-1.5 rounded-2xl shadow-2xl border border-white/5 flex flex-col gap-1">
          <RenderToggle active={renderSettings.sunlight} onClick={() => setRenderSettings({ sunlight: !renderSettings.sunlight })} icon={<IconSun className="w-3.5 h-3.5" />} label="Sunlight" />
          <RenderToggle active={renderSettings.shadows} disabled={!renderSettings.sunlight} onClick={() => setRenderSettings({ shadows: !renderSettings.shadows })} icon={<IconMoon className="w-3.5 h-3.5" />} label="Shadows" />
          <div className="h-px bg-white/5 mx-2 my-0.5" />
          <RenderToggle active={renderSettings.pbr} onClick={() => setRenderSettings({ pbr: !renderSettings.pbr })} icon={<IconLayers className="w-3.5 h-3.5" />} label="PBR Materials" />
          <RenderToggle active={renderSettings.wallTransparency} onClick={() => setRenderSettings({ wallTransparency: !renderSettings.wallTransparency })} icon={<IconEyeOff className="w-3.5 h-3.5" />} label="X-Ray Walls" />
        </div>
      </div>

      {/* Scene content — only when ready */}
      {sceneReady && (
